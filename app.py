...
# ─────────────────────────────────────────────
# PRE-COMPUTE SHARED VARIABLES (used across tabs)
# ─────────────────────────────────────────────
...
with tabs[7]:
    ...
    st.markdown('<div class="section-head">🧩 K-Means Consumer Segmentation</div>', unsafe_allow_html=True)
    st.caption("Cluster respondents into behavioral segments based on engagement, spending, and purchase behavior.")

    cluster_feature_cols = ['spending_ord', 'daily_time_ord', 'engagement_ord',
                             'product_interest_score', 'satisfaction_score',
                             'ad_click_ord', 'retargeting_score']
    cluster_labels_list = ['Spending', 'Daily Time', 'Engagement',
                            'Product Interest', 'Satisfaction', 'Ad Clicks', 'Retargeting']

    cluster_data = filtered[cluster_feature_cols].dropna()

    if len(cluster_data) < 20:
        st.warning("Not enough data for clustering. Please widen your filters.")
    else:
        sc_clust = StandardScaler()
        X_clust = sc_clust.fit_transform(cluster_data)
        ...
        with col_cl1:
            ...
            ax.plot(list(k_range), inertias, 'o-', color=PALETTE[0], linewidth=2, label='Inertia')
            ax.set_xlabel('K'); ax.set_ylabel('Inertia', color=PALETTE[0])
            ax2_elbow = ax.twinx()
            ax2_elbow.plot(list(k_range), silhouettes, 's--', color=PALETTE[1], linewidth=2, label='Silhouette')
            ax2_elbow.set_ylabel('Silhouette Score', color=PALETTE[1])
            ax.set_title('Elbow & Silhouette', fontweight='bold', fontsize=9)
            st.pyplot(fig, use_container_width=True)
            plt.close()
            best_k = list(k_range)[np.argmax(silhouettes)]
            st.caption(f"Best K by silhouette: **{best_k}**")
        ...
        with col_cl2:
            ...
            X_cl_2d = pca_cl.fit_transform(X_clust)

            fig, ax = plt.subplots(figsize=(7, 5))
            for c in range(n_clusters):
                mask = cluster_labels == c
                ax.scatter(X_cl_2d[mask, 0], X_cl_2d[mask, 1],
                           c=PALETTE[c % len(PALETTE)], alpha=0.65, s=45, label=f'Cluster {c+1}')
            ...
            ax.set_title('Consumer Clusters (PCA 2D Projection)', fontweight='bold')
            ax.legend()
            sil = silhouette_score(X_clust, cluster_labels)
            ax.set_title(f'Consumer Clusters — Silhouette Score: {sil:.3f}', fontweight='bold')
            st.pyplot(fig, use_container_width=True)
            plt.close()
        ...
        cluster_df['purchase_ord'] = filtered['purchase_ord'].loc[cluster_data.index].values
        cluster_df['made_purchase'] = filtered['made_purchase'].loc[cluster_data.index].values
        cluster_df['high_ad_responsive'] = filtered['high_ad_responsive'].loc[cluster_data.index].values

        profile = cluster_df.groupby('Cluster').agg(
            Count=('made_purchase', 'count'),
            Avg_Spending=('spending_ord', 'mean'),
            Avg_Engagement=('engagement_ord', 'mean'),
            Avg_Interest=('product_interest_score', 'mean'),
            Avg_Satisfaction=('satisfaction_score', 'mean'),
            Purchase_Rate=('made_purchase', 'mean'),
            Ad_Responsive_Rate=('high_ad_responsive', 'mean'),
        ).round(3)
        profile['Purchase_Rate'] = (profile['Purchase_Rate'] * 100).round(1).astype(str) + '%'
        profile['Ad_Responsive_Rate'] = (profile['Ad_Responsive_Rate'] * 100).round(1).astype(str) + '%'

        st.dataframe(profile.style.background_gradient(cmap='plasma', subset=['Avg_Engagement', 'Avg_Interest']),
                     use_container_width=True)

        # Radar chart per cluster
        st.markdown('<div class="section-head">Cluster Radar Comparison</div>', unsafe_allow_html=True)

        radar_cols = cluster_feature_cols
        radar_labels = cluster_labels_list
        centers = sc_clust.inverse_transform(km_final.cluster_centers_)
        ...
        for c in range(n_clusters):
            vals = centers[c].tolist()
            vals += vals[:1]
            ax.plot(angles, vals, '-o', color=PALETTE[c % len(PALETTE)], linewidth=2, label=f'Cluster {c+1}')
            ax.fill(angles, vals, alpha=0.08, color=PALETTE[c % len(PALETTE)])

        ax.set_xticks(angles[:-1])
        ax.set_xticklabels(radar_labels, fontsize=9)
        ax.set_title('Cluster Radar Chart (Original Scale)', fontweight='bold', pad=20)
        ax.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1), fontsize=9)
        st.pyplot(fig, use_container_width=True)
        plt.close()

        # Cluster naming insight
        st.markdown('<div class="section-head">Cluster Interpretations</div>', unsafe_allow_html=True)
        cluster_names = []
        for c in range(n_clusters):
            cdata = cluster_df[cluster_df['Cluster'] == c + 1]
            eng = cdata['engagement_ord'].mean()
            spend = cdata['spending_ord'].mean()
            pur = cdata['made_purchase'].mean()
            if pur > 0.85 and eng > 2.5:
                name = "🔥 High-Value Converter"
            elif pur > 0.7 and eng <= 2.5:
                name = "💤 Passive Buyer"
            elif pur <= 0.5 and eng > 2:
                name = "👀 Engaged Window-Shopper"
            else:
                name = "❄️ Low-Engagement Observer"
            cluster_names.append(name)
            st.markdown(f"""
            <div class="insight-box">
              <div class="insight-title" style="color:{PALETTE[c % len(PALETTE)]}">
                Cluster {c+1}: {name}
              </div>
              <div class="insight-text">
                n={len(cdata)} &nbsp;|&nbsp; Avg Engagement: {eng:.2f} &nbsp;|&nbsp;
                Avg Spending: {spend:.2f} &nbsp;|&nbsp; Purchase Rate: {pur*100:.1f}%
              </div>
            </div>
            """, unsafe_allow_html=True)
